...
Preprocessor.prototype.sass = function () {
  ...
  if(this.options.in && Array.isArray(this.options.in)){
    opts.includePaths = [];
    for (var item in this.options.in) {
      opts.includePaths.push(path.dirname(this.options.in[item]));
    }
    opts.file = this.options.in[0];

  }else if(this.options.in){
    opts.includePaths = [path.dirname(item)];
    opts.file = this.options.in;
  }

  opts.data = this.options.sass.data;
  opts.dest = path.dirname(opts.file);
